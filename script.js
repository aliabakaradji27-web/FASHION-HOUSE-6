...
async function login() {
  try {
    ...
    const auth = await Sidra.authenticate(scopes, onIncompletePaymentFound);
...
function renderCart() {
  ...
  cart.forEach(item => {
    container.innerHTML += <p>${item.title} - ${item.price} FASHION HOUSE;
    total += item.price;
  });
...
function checkout() {
  ...
  Pi.createPayment  amount: total,  memo: "FASHION HOUSE Order",
    metadata: cart
  }, {
